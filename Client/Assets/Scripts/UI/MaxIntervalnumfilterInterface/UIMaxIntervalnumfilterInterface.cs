...
{
    class UIMaxIntervalnumfilterInterface : UIBase
    {
        ...
        {
            //----------------------------------------------------------------------------
            UIMaxIntervalnumfilterInterface m_parent;
            ...
            //----------------------------------------------------------------------------
            public SumTemp(GameObject go, UIMaxIntervalnumfilterInterface _parent, int numType)
            {
                m_ObjUI = go;
        ...
        {
            base.Show(arg);
            maxIntervalNumberData = drawData.maxIntervalNumberData;
        ...
        public void SetSumPanelView()
        {
            ...
            dictor = from objDic in maxIntervalNumberData.maxIntervalNumberDict orderby objDic.Value descending select objDic;
            int i = 0;
        ...
        private void OnBack()
        {
            ...
            Messenger.Broadcast(DgMsgID.DgUI_HideNew, "UIMaxIntervalnumfilterInterfaceCtrl");
        }
        //----------------------------------------------------------------------------
        private void OnBegin()
        {
            ...
            {
                drawData.resultList = Util.GetMaxIntervalResult(initialFilterResults, selectTypeList);
                m_totalAfter.text = string.Format(PromptData.GetPrompt("afterFilterTotal"), drawData.resultList.Count);
                UpdateLoopView(drawData.resultList, afterLoopScrollView);
            }
        }
