...
{
    class UISumvalueNumFilterInterface : UIBase
    {
        ...
        {
            //----------------------------------------------------------------------------
            UISumvalueNumFilterInterface m_parent;
            ...
            //----------------------------------------------------------------------------
            public SumTemp(GameObject go, UISumvalueNumFilterInterface _parent, int numType)
            {
                m_ObjUI = go;
            ...
            public void SetView(int count)
            {
                ...
                m_goRect.sizeDelta += new Vector2(0, count / 5.0f);
            }
            //----------------------------------------------------------------------------
        ...
        {
            base.Show(arg);
            sumvalueNumberData = drawData.sumvalueNumberData;
        ...
        public void SetSumPanelView()
        {
            ...
            dictor = from objDic in sumvalueNumberData.sumvalueNumberDict orderby objDic.Key select objDic;
            int i = 0;
        ...
        {
            Hide();      
            Messenger.Broadcast<string, object>(DgMsgID.DgUI_ShowNewOneParam, "UISerialNumFilterInterfaceCtrl", drawData.resultList);
        }
        //----------------------------------------------------------------------------
        ...
        private void OnNormalToggleChange(bool isOn)
        {
            ...
            {
                int i = 0;
                dictor = from objDic in sumvalueNumberData.sumvalueNumberDict orderby objDic.Value descending select objDic;
                foreach (var item in dictor)
                {
                    i++;
                    for (int j = 0; j < m_SumTemp.Count; j++)
        ...
        private void OnBack()
        {
            ...
            Messenger.Broadcast(DgMsgID.DgUI_HideNew, "UISumvalueNumFilterInterfaceCtrl");
        }
        //----------------------------------------------------------------------------
        private void OnBegin()
        {
            ...
            {
                drawData.resultList = Util.GetSumvalueFilterResult(initialFilterResults, selectTypeList);
                m_totalAfter.text = string.Format(PromptData.GetPrompt("afterFilterTotal"), drawData.resultList.Count);
                UpdateLoopView(drawData.resultList, afterLoopScrollView);
            }
        }
