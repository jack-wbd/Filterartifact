...
{
    class UISerialNumFilterInterface : UIBase
    {
        ...
        {
            //----------------------------------------------------------------------------
            UISerialNumFilterInterface m_parent;
            ...
            //----------------------------------------------------------------------------
            public SumTemp(GameObject go, UISerialNumFilterInterface _parent, int numType)
            {
                m_ObjUI = go;
        ...
        public void SetSumPanelView()
        {
            ...
            dictor = from objDic in serialNumberData.serialNumberDict orderby objDic.Key select objDic;
            int i = 0;
        ...
        {
            Hide();
            Messenger.Broadcast<string, object>(DgMsgID.DgUI_ShowNewOneParam, "UIDoubleNumFilterInterfaceCtrl", drawData.resultList);
        }
        //----------------------------------------------------------------------------
        ...
        private void OnNormalToggleChange(bool isOn)
        {
            ...
            {
                int i = 0;
                dictor = from objDic in serialNumberData.serialNumberDict orderby objDic.Value descending select objDic;
                foreach (var item in dictor)
                {
                    i++;
                    for (int j = 0; j < m_SumTemp.Count; j++)
        ...
        private void OnBack()
        {
            ...
            Messenger.Broadcast(DgMsgID.DgUI_HideNew, "UISerialNumFilterInterfaceCtrl");
        }
        //----------------------------------------------------------------------------
        private void OnBegin()
        {
            ...
            {
                drawData.resultList = Util.GetSerialFilterResult(initialFilterResults, selectTypeList);
                m_totalAfter.text = string.Format(PromptData.GetPrompt("afterFilterTotal"), drawData.resultList.Count);
                UpdateLoopView(drawData.resultList, afterLoopScrollView);
            }
        }
