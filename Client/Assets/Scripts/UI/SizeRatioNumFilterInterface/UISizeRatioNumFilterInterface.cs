...
{
    class UISizeRatioNumFilterInterface : UIBase
    {
        ...
        private SizeRatioNumberData sizeRatioNumberData;
        ...
        {
            //----------------------------------------------------------------------------
            UISizeRatioNumFilterInterface m_parent;
            ...
            //----------------------------------------------------------------------------
            public SumTemp(GameObject go, UISizeRatioNumFilterInterface _parent, string numType)
            {
                m_ObjUI = go;
        ...
        {
            base.Show(arg);
            sizeRatioNumberData = drawData.sizeRatioNumberData;
        ...
        public void SetSumPanelView()
        {
            ...
            dictor = from objDic in sizeRatioNumberData.sizeRatioNumberDict orderby objDic.Key select objDic;
            int i = 0;
        ...
        {
            Hide();          
            Messenger.Broadcast<string, object>(DgMsgID.DgUI_ShowNewOneParam, "UIRedeemNumfilterInterfaceCtrl", drawData.resultList);
        }
        //----------------------------------------------------------------------------
        ...
        private void OnNormalToggleChange(bool isOn)
        {
            ...
            {
                int i = 0;
                dictor = from objDic in sizeRatioNumberData.sizeRatioNumberDict orderby objDic.Value descending select objDic;
                foreach (var item in dictor)
                {
                    i++;
                    for (int j = 0; j < m_SumTemp.Count; j++)
        ...
        private void OnBegin()
        {
            ...
            {
                drawData.resultList = Util.GetSizeRatioFilterResult(initialFilterResults, selectTypeList);
                m_totalAfter.text = string.Format(PromptData.GetPrompt("afterFilterTotal"), drawData.resultList.Count);
                UpdateLoopView(drawData.resultList, afterLoopScrollView);
            }
        }
